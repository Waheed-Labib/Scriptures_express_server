...
const getTranslation = asyncHandler(async (req, res) => {
    const { userId, verse_key } = req.query;

    if (!verse_key) {
        throw new ApiError(400, "Verse Key not sent")
    }
...
const addTranslation = asyncHandler(async (req, res) => {
    ...
    if (alreadyExists) {
        throw new ApiError(401, "You have already translated this verse. Please try to edit existing translation.")
    }
